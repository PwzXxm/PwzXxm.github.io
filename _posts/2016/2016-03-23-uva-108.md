...
Then as we can see at the graph below, it is obvious that \\(A = (A+B+C+D)-(B+D)-(C+D)+D\\). Therefore we have `A = sum[k][l] - sum[i-1][l] - sum[j-1][k] + sum[i-1][j-1]`. Go through every subarray to find the maximum.
